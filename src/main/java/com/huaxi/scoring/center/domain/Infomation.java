...
//    @Size(max=50)
	...
	@NotEmpty(message = "报价不能为空")
	@Column(nullable = false)
	private int price;

	@NotEmpty(message = "最终报价不能为空")
	@Column(nullable = false)
	private int fprice;
	...
	protected Infomation() { // 无参构造函数;设为 protected 防止直接使用
	}


	public Infomation(Long id,@NotEmpty(message = "公司名称不能为空") String company, @NotEmpty(message = "产地及品牌不能为空") String origin, @NotEmpty(message = "规格型号不能为空") String type, @NotEmpty(message = "报价不能为空") int price, @NotEmpty(message = "最终报价不能为空") int fprice, String comment, Date date, @NotEmpty(message = "地点不能为空") String location, @NotEmpty(message = "记录人不能为空") String recoder, @NotEmpty(message = "复核不能为空") String reviewer, @NotEmpty(message = "经办人不能为空") String responer, @NotEmpty(message = "议价内容不能为空") String bargain) {
		this.id = id;
